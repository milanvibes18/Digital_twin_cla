for value_col in value_cols:
                ...
                # Get data with location and values
                analysis_data = data[location_cols + [value_col]].dropna()
                
                if len(analysis_data) < 10:
                    continue
                
                values = analysis_data[value_col]
                ...
                hotspot_threshold = q75 + 1.5 * iqr
                coldspot_threshold = q25 - 1.5 * iqr
                
                hotspot_mask = values > hotspot_threshold
                coldspot_mask = values < coldspot_threshold
                
                hotspot_locations = analysis_data[hotspot_mask][location_cols + [value_col]]
                coldspot_locations = analysis_data[coldspot_mask][location_cols + [value_col]]
                
                # Calculate hotspot statistics
                hotspot_stats = {
                    'count': len(hotspot_locations),
                    'threshold': float(hotspot_threshold),
                    'locations': hotspot_locations.to_dict('records') if len(hotspot_locations) <= 20 else [],
                    'mean_value': float(hotspot_locations[value_col].mean()) if len(hotspot_locations) > 0 else 0
                }
                
                coldspot_stats = {
                    'count': len(coldspot_locations),
                    'threshold': float(coldspot_threshold),
                    'locations': coldspot_locations.to_dict('records') if len(coldspot_locations) <= 20 else [],
                    'mean_value': float(coldspot_locations[value_col].mean()) if len(coldspot_locations) > 0 else 0
                }
                
                hotspots[value_col] = {
                    'hotspots': hotspot_stats,
                    'coldspots': coldspot_stats,
                    'analysis_parameters': {
                        'hotspot_threshold': float(hotspot_threshold),
                        'coldspot_threshold': float(coldspot_threshold),
                        'q25': float(q25),
                        'q75': float(q75),
                        'iqr': float(iqr)
                    }
            ...
            self.logger.error(f"Spatial hotspot analysis error: {e}")
            return {}
    
    def _calculate_spatial_autocorrelation(self, data: pd.DataFrame, 
        ...
        try:
            ...
            for value_col in value_cols:
                ...
                locations = analysis_data[location_cols].values
                ...
                with np.errstate(divide='ignore', invalid='ignore'):
                    weights = 1.0 / distance_matrix
                    weights[distance_matrix == 0] = 0  # Set diagonal to 0
                ...
                # Calculate Moran's I
                values = analysis_data[value_col].values
                n = len(values)
                ...
                if denominator > 0:
                    ...
                    # Z-score (simplified calculation)
                    variance_i = 2.0 / ((n - 1) * (n - 2))  # Approximate
                    z_score = (morans_i - expected_i) / np.sqrt(variance_i)
                    
                    autocorrelation_results[value_col] = {
            ...
            self.logger.error(f"Spatial autocorrelation calculation error: {e}")
            return {}
    
    def _interpret_morans_i(self, morans_i: float) -> str:
        """Interpret Moran's I statistic."""
        if morans_i > 0.1:
            return "positive_autocorrelation"  # Similar values cluster together
        elif morans_i < -0.1:
            return "negative_autocorrelation"  # Dissimilar values cluster together
        else:
            return "no_autocorrelation"  # Random spatial distribution
    
    def analyze_behavioral_patterns(self, data: pd.DataFrame, 
                                   entity_col: str, 
                                   behavior_cols: List[str] = None,
                                   timestamp_col: str = None) -> Dict:
        """
        Analyze behavioral patterns in entity data.
        
        Args:
            data: DataFrame with behavioral data
            entity_col: Column identifying entities (devices, users, etc.)
            behavior_cols: Columns representing behaviors
        ...
        try:
            ...
            if behavior_cols is None:
                behavior_cols = df.select_dtypes(include=[np.number]).columns.tolist()
                behavior_cols = [col for col in behavior_cols if col != entity_col]
            ...
            # 1. Entity clustering based on behavior
            entity_clustering = self._analyze_entity_clustering(df, entity_col, behavior_cols)
            results['behavioral_patterns']['entity_clustering'] = entity_clustering
            
            # 2. Behavioral sequence analysis
            if timestamp_col:
                sequence_analysis = self._analyze_behavioral_sequences(df, entity_col, behavior_cols, timestamp_col)
                results['behavioral_patterns']['sequences'] = sequence_analysis
            
            # 3. Anomalous behavior detection
            anomalous_behavior = self._detect_anomalous_behavior(df, entity_col, behavior_cols)
            results['behavioral_patterns']['anomalies'] = anomalous_behavior
            
            # 4. Behavioral state transitions
            if timestamp_col:
                state_transitions = self._analyze_state_transitions(df, entity_col, behavior_cols, timestamp_col)
                results['behavioral_patterns']['state_transitions'] = state_transitions
            
            # 5. Collective behavior patterns
            collective_behavior = self._analyze_collective_behavior(df, entity_col, behavior_cols)
            results['behavioral_patterns']['collective'] = collective_behavior
    ...
    def _analyze_entity_clustering(self, data: pd.DataFrame, 
                                  entity_col: str, 
                                  behavior_cols: List[str]) -> Dict:
        """Cluster entities based on behavioral patterns."""
        try:
            # Aggregate behavior data by entity
            entity_profiles = data.groupby(entity_col)[behavior_cols].agg(['mean', 'std', 'min', 'max']).fillna(0)
            
            # Flatten multi-level columns
            entity_profiles.columns = [f"{col[0]}_{col[1]}" for col in entity_profiles.columns]
            
            if len(entity_profiles) < 3:
                return {'clustering': False, 'reason': 'Insufficient entities'}
            ...
            optimal_k = self._find_optimal_clusters_behavioral(profiles_scaled)
            ...
            for cluster_id in range(optimal_k):
                ...
                cluster_entities = entity_profiles[cluster_mask]
                cluster_entity_ids = cluster_entities.index.tolist()
                ...
                cluster_analysis[f'cluster_{cluster_id}'] = {
                    ...
                    'entities': cluster_entity_ids[:20],  # Limit for readability
                    'characteristics': dict(zip(entity_profiles.columns, cluster_center_original.tolist())),
                    'behavior_summary': self._summarize_cluster_behavior(cluster_entities, behavior_cols)
                }
            ...
            return {
                ...
                'silhouette_score': float(silhouette_score(profiles_scaled, cluster_labels)) if len(set(cluster_labels)) > 1 else 0
            }
            
        except Exception as e:
            self.logger.error(f"Entity clustering error: {e}")
            return {'clustering': False, 'reason': str(e)}
    ...
    def _analyze_state_transitions(self, data: pd.DataFrame, 
        ...
        try:
            ...
            for col in behavior_cols:
                ...
                # Analyze transitions for each entity
...
if __name__ == "__main__":
    ...
    print("- All patterns saved for future reference and comparison")import numpy as np
import pandas as pd
import logging
import json
import pickle
...
# Visualization (for pattern visualization)
...
class PatternAnalyzer:
    ...
    def analyze_spatial_patterns(self, data: pd.DataFrame, 
        ...
        try:
            ...
            if value_cols is None:
                value_cols = df.select_dtypes(include=[np.number]).columns.tolist()
                # Remove location columns from value columns
                value_cols = [col for col in value_cols if col not in location_cols]
            ...
            if len(location_cols) >= 2:
                spatial_clustering = self._analyze_spatial_clustering(df, location_cols, value_cols)
                results['spatial_patterns']['clustering'] = spatial_clustering
            
            # 2. Spatial correlation analysis
            spatial_correlation = self._analyze_spatial_correlation(df, value_cols)
            results['spatial_patterns']['correlation'] = spatial_correlation
            
            # 3. Spatial hotspot analysis
            if location_cols and value_cols:
                hotspot_analysis = self._analyze_spatial_hotspots(df, location_cols, value_cols)
                results['spatial_patterns']['hotspots'] = hotspot_analysis
            
            # 4. Spatial autocorrelation
            if len(location_cols) >= 2 and value_cols:
                autocorr = self._calculate_spatial_autocorrelation(df, location_cols, value_cols)
                results['spatial_patterns']['autocorrelation'] = autocorr
            ...
            self.logger.error(f"Spatial pattern analysis error: {e}")
            raise
    
    def _analyze_spatial_clustering(self, data: pd.DataFrame, 
                                   location_cols: List[str], 
                                   value_cols: List[str]) -> Dict:
        """Analyze spatial clustering patterns."""
        try:
            # Extract location data
            location_data = data[location_cols].dropna()
            
            if len(location_data) < 10:
                return {'clustering': False, 'reason': 'Insufficient location data'}
            ...
            cluster_labels = clustering.fit_predict(location_scaled)
            ...
            for cluster_id in set(cluster_labels):
                ...
                cluster_mask = cluster_labels == cluster_id
                cluster_points = location_data[cluster_mask]
                cluster_values = data.loc[cluster_points.index, value_cols]
                
                # Calculate cluster statistics
                centroid = cluster_points.mean().to_dict()
                size = len(cluster_points)
                
                # Value statistics for this cluster
                value_stats = {}
                for col in value_cols:
                    if col in cluster_values.columns:
                        col_data = cluster_values[col].dropna()
                        if len(col_data) > 0:
                            value_stats[col] = {
            ...
            self.logger.error(f"Spatial clustering analysis error: {e}")
            return {'clustering': False, 'reason': str(e)}
    
    def _analyze_spatial_correlation(self, data: pd.DataFrame, value_cols: List[str]) -> Dict:
        """Analyze spatial correlation patterns."""
        try:
            # Calculate correlation matrix
            correlation_data = data[value_cols].dropna()
            
            if len(correlation_data) < 10:
                return {'correlation_analysis': False, 'reason': 'Insufficient data'}
            
            corr_matrix = correlation_data.corr()
            
            # Find strong correlations
            strong_correlations = []
            correlation_threshold = self.config['spatial_patterns']['correlation_threshold']
            ...
            for i, col1 in enumerate(value_cols):
                for j, col2 in enumerate(value_cols[i+1:], i+1):
                    if col1 in correlation_data.columns and col2 in correlation_data.columns:
                        col1_data = correlation_data[col1].dropna()
                        col2_data = correlation_data[col2].dropna()
                        ...
                        if len(common_idx) > 10:
                            try:
                                mi_score = mutual_info_regression(
                                    col1_data.loc[common_idx].values.reshape(-1, 1),
                                    col2_data.loc[common_idx].values
                                )[0]
                                mutual_info[f"{col1}_{col2}"] = float(mi_score)
                            except:
                                continue
            ...
            self.logger.error(f"Spatial correlation analysis error: {e}")
            return {'correlation_analysis': False, 'reason': str(e)}
    
    def _analyze_spatial_hotspots(self, data: pd.DataFrame, 
                                 location_cols: List[str], 
                                 value_cols: List[str]) -> Dict:
        """Analyze spatial hotspots and coldspots."""
        try:
            hotspots = {}
